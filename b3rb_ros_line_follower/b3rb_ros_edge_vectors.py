...
VECTOR_IMAGE_HEIGHT_30_PERCENTAGE = 0.30  # Bottom portion of image to be analyzed for vectors.
VECTOR_IMAGE_HEIGHT_20_PERCENTAGE = 0.30
VECTOR_MAGNITUDE_MINIMUM = 15
...
class EdgeVectorsPublisher(Node):
	...
	def process_image_for_edge_vectors(self, image):
		...
		if self.obstacles is True:
			self.lower_image_height = int(self.image_height * VECTOR_IMAGE_HEIGHT_30_PERCENTAGE)
		else:
			self.lower_image_height = int(self.image_height * VECTOR_IMAGE_HEIGHT_20_PERCENTAGE)
		...
		image_height, image_width = out_binary.shape
		thresh = cv2.bitwise_not(out_binary)
